...
from app.karlo_c.schemas import contact_schema
from app.karlo_c.services.contact import contact_service
...
def get_contact(id: int, request: Request, db: Session = Depends(get_db)):
...
def get_all_contacts(request: Request, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    require_superuser(request, db)
...
def delete_contact(id: int, request: Request, db: Session = Depends(get_db)):
    require_superuser(request, db)
...
def get_contact_by_email(email: str, request: Request, db: Session = Depends(get_db)):
    require_superuser(request, db)
